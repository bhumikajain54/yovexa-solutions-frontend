...
export default function AdminAboutListPage() {
  ...
  const loadAbouts = async () => {
    try {
      ...
      showToast('Failed to load about records', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSetActive = async (id, currentStatus) => {
    ...
    try {
      ...
      showToast('About section activated on public homepage! Previous section deactivated.', 'success');
      await loadAbouts();
    } catch (err) {
      console.error('Failed to set active about:', err);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      await aboutService.deleteAbout(deleteTarget.id);
  ...
  const publishedCount = abouts.filter(a => a.status === 'PUBLISHED').length;
  const draftCount = abouts.filter(a => a.status === 'DRAFT').length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-[#E2E8F0]">
        <div>
          ...
          <p className="text-sm text-[#475569] mt-0.5 font-medium">
            Create and manage About section content for the homepage.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-[#E2E8F0] shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-[#0B1B3A] mt-1">{abouts.length}</div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 text-center flex flex-col items-center justify-center space-y-3">
            ...
            <p className="text-sm font-semibold text-[#64748B]">Loading About sections...</p>
          </div>
        ) : abouts.length === 0 ? (
          <div className="p-12 text-center flex flex-col items-center justify-center space-y-4">
            ...
            <div>
              <h3 className="text-base font-bold text-[#0B1B3A]">No About sections found.</h3>
              <p className="text-xs text-[#64748B] mt-1 max-w-sm mx-auto">
                Create a new About section record to manage company narrative, mission, and highlights.
              </p>
            </div>
            <Link
              to="/admin/content/about/create"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#E2E8F0] bg-[#F8FAFC] text-[11px] font-bold text-[#64748B] uppercase tracking-wider">
                  ...
                  <th className="py-3.5 px-4">Eyebrow</th>
                  <th className="py-3.5 px-4">Main Title</th>
                  <th className="py-3.5 px-4 hidden md:table-cell">Highlighted Title</th>
              ...
              <tbody className="divide-y divide-[#E2E8F0] text-sm">
                {abouts.map((about) => {
                  const isCurrentActive = Boolean(about.isActive);
                    ...
                    >
                      ...
                      <td className="py-4 px-4 sm:px-6">
                        <div className="flex flex-col gap-1 items-start">
                          {isCurrentActive ? (
                            ...
                            <button
                              onClick={() => handleSetActive(about.id, about.status)}
                              disabled={activatingId === about.id || !isPublished}
                      ...
                      <td className="py-4 px-4">
                        <span className="inline-block px-2.5 py-1 rounded-lg bg-[#0EA5E9]/10 text-[#0284C7] text-xs font-bold max-w-[180px] truncate">
                          {about.sectionLabel || about.eyebrow || '—'}
                        </span>
                      </td>

                      {/* Main Title */}
                      <td className="py-4 px-4 font-bold text-[#0B1B3A] max-w-[240px]">
                        <div className="truncate" title={about.title}>
                          {about.title}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4 hidden md:table-cell text-xs font-semibold text-[#0284C7] max-w-[200px]">
                        <div className="truncate" title={about.titleHighlight || about.highlightedTitle}>
                      ...
                      <td className="py-4 px-4 hidden lg:table-cell text-xs text-[#64748B]">
                        {about.updatedAt ? new Date(about.updatedAt).toLocaleDateString('en-US', {
                          month: 'short',
                          day: 'numeric',
                          year: 'numeric'
                        }) : '—'}
                      ...
                      <td className="py-4 px-4 sm:px-6 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <Link
                            to={`/admin/content/about/${about.id}`}
          ...
          deleteTarget?.isActive ? (
            <div className="space-y-2">
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-rose-800 text-xs font-bold flex items-center gap-2">
                ...
                <span>Warning: This About section is currently ACTIVE on the public homepage!</span>
              </div>
              <p>
                Deleting it may leave the homepage without About content until another variation is activated. Continue?
              </p>
            </div>
          ) : (
            `Are you sure you want to delete the About section "${deleteTarget?.title}"? This action cannot be undone.`
          )
        }
